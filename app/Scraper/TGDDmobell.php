...
class TGDDmobell
{
    ...
    {
        $url = "http://api.scraperapi.com?api_key=db2edb40264fd762f0ce7f4e082be6ae&url=https://www.thegioididong.com/dtdd-mobell";

        $client = new Client();

        $crawler = $client->request('GET', $url);
        if (Brand_phone::where('name', '=', 'Mobell')->count() == 0) {
            $newbrandphone = new Brand_phone;
            $newbrandphone->name = 'Mobell';
            $newbrandphone->save();
        }
        $brand = Brand_phone::where('name', 'Mobell')->first();
        ...
        $crawler->filter('ul.listproduct li.item')->each(
            function (Crawler $node) use ($brand) {
                try {
                    ...
                    $urlphone = "http://api.scraperapi.com?api_key=db2edb40264fd762f0ce7f4e082be6ae&url=https://www.thegioididong.com" . $dataurl;
                        ...
                        function (Crawler $node) use ($image, $brand) {
                            ...
                            if ($node->filter('div.box_main > div.box_right')->children('div.scrolling_inner')->count() > 1) {
                                ...
                                for ($i = 0; $i < $countbn; $i++) {
                                    ...
                                    $urlbn = "http://api.scraperapi.com?api_key=db2edb40264fd762f0ce7f4e082be6ae&url=https://www.thegioididong.com" . $bnlink;
                                    $bnclient = new Client();
