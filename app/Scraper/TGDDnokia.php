...
class TGDDnokia
{
    ...
    {
        $url = "http://api.scraperapi.com?api_key=db2edb40264fd762f0ce7f4e082be6ae&url=https://www.thegioididong.com/dtdd-nokia";

        $client = new Client();

        $crawler = $client->request('GET', $url);
        if (Brand_phone::where('name', '=', 'Nokia')->count() == 0) {
            $newbrandphone = new Brand_phone;
            $newbrandphone->name = 'Nokia';
            $newbrandphone->save();
        }
        $brand = Brand_phone::where('name', 'Nokia')->first();
