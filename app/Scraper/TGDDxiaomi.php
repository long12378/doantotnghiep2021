...
class TGDDxiaomi
{
    ...
    {
        $url = "http://api.scraperapi.com/?api_key=6a620ba9d3253bed8983b4c0a2a1c332&url=https://www.thegioididong.com/dtdd-xiaomi";

        $client = new Client();

        $crawler = $client->request('GET', $url);
        if (Brand_phone::where('name', '=', 'Xiaomi')->count() == 0) {
            $newbrandphone = new Brand_phone;
            $newbrandphone->name = 'Xiaomi';
            $newbrandphone->save();
        }
        $brand = Brand_phone::where('name', 'Xiaomi')->first();
